defmodule RemindWeb.UserAuthTest do
  use RemindWeb.ConnCase, async: true

  alias Remind.Account
  alias RemindWeb.UserAuth
  import Remind.AccountFixtures

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, RemindWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    test "stores the user token in the session", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == "/remindres"
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      RemindWeb.Endpoint.subscribe(live_socket_id)
